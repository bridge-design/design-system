...
const ProfileCard = ({ photoUrl, name, role, description, twitter, linkedin, github, reverse }) => {
  ...
  return (
    <div
      className={`flex items-start flex-col gap-4 rounded-[4px] bg-light-on-background-200 lg:max-h-min min-w-min lg:max-w-full overflow-hidden justify-between px-4 py-4 sm:px-0 sm:py-0 ${
        reverse ? "sm:flex-row-reverse lg:pl-6 sm:pl-4" : "sm:flex-row"
      }`}
    >
      <img src={photoUrl} alt={name} className="rounded-l-[4px] max-w-[306px] object-contain" />
      <div
        className={
          "lg:min-h-[342px] flex flex-col justify-between w-full sm:py-4 lg:pl-6 xl:py-10 lg:pr-10"
        }
      >
        <Text
          variant="2XlMedium"
